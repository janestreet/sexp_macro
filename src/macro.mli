...
(** This module implements a limited form of evaluation within s-expressions, usually
    ...
    - [(:include filename)] expands to the list of s-expressions contained in [filename],
      as if the contents of [filename] were directly inserted in place of
      [(:include filename)]. A relative [filename] is taken with respect to the file that
      contains the include macro.
    - [(:let v (v1 ... vn) S1 ... Sm)] defines a macro-variable [v] with labelled
      parameters [v1, ..., vn] and body [S1 ... Sm]. The definition itself expands to
      nothing. All macro-variables ([v] and [v1 .. vn] here) are lexically scoped. An
      error is reported if [S1 ... Sm] is empty. .
    - [(:use v (v1 SS1) ... (vn SSn))] calls the macro-variable [v] with the expansions of
      ...
      parameters [v1, ..., vn] of [v]. Parameters take no arguments. An error is reported
      if [v] is not in scope, or the parameter labels do not match.
    ...
    Macros other than [:include] will be called 'local'. All [:include] macros are
    expanded before all local macros, so [(:include (:use x))] is always an error and if
    file1 includes file2 and file2 include file3, file1 can use templates defined in
    file3. However files can only use macro-variables that they brought in scope: if file1
    defines a macro-variable [a] and includes [file2], file2 cannot use [a].
...
(** A version of [load_sexps] that is functorized with respect to the functions that load
    the sexps from files and the corresponding monad. *)
